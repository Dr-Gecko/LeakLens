...
async def reload_count_stream(request: Request):
    ...
    verified = await auth.verifyAuthRole(auth_token)
...
async def searchAllColumns(table_name: str, search_value: str, limit: int = 100, search_field: str = None):
    try:
        breach_database = config.get_config_value("database.breaches_db")
        ...
        if not columns: return utils.formatResponse(data=[],status_code=404,reason="table_not_found")
        column_names = [row["column_name"] for row in columns]
        if search_field:
            if search_field not in column_names: return utils.formatResponse(data=[],status_code=400,reason="invalid_field")
        ...
        return utils.formatResponse(data={"breach_data": breach_meta, "entries": utils.clean_json([parse_extra(dict(row)) for row in rows])})
    except Exception as error: 
        print(error)
        traceback.print_exc()
